...
export class ReportComponent implements OnInit {
	...
	public optionsPie: ChartOptions = {
	...
	getMaterialColors(){
		...
		const randomColor = Math.random() * (Colors.length - 1);
		return Colors[Math.floor(randomColor)];
	}
	...
	ngOnInit(): void {
		...
		console.log(this.storage.getColors());
		...
		this.careerInfoList = this.stidentInfoList.reduce((ac, i) => {
			...
			return ac;
		}, [] as ICareer[]).sort((a, b) => (b.totalStundets) - (a.totalStundets));
		...
		this.careerInfoList.forEach(x => {
			...
			colors.push(this.getMaterialColors());
